@component('layouts.page', ['card_header' => 'Edit Application'])
    @slot('card_body')
        ...
        <form id="authorize-form" action="{{ route('authorize.update', $application->id) }}" method="POST">
            @csrf
            @method('PUT')
            <hr>
            @include('authorize.partials.peralatan')
            
            <hr>
            @include('authorize.partials.approval')

            <hr>
            @include('authorize.partials.authorize')
        </form>

    @endslot
    @slot('card_footer')
        <a href="{{ route('authorize.index') }}" class="btn btn-default border-primary">Back</a>
        <div class="btn btn-primary float-right" 
            onclick="document.getElementById('authorize-form').submit()">
            Update
        </div>
    @endslot
@endcomponent